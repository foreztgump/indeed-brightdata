...
# Mock curl that returns a sequence of responses (one per call)
# Usage: create_curl_sequence_mock '{"status":"ready"}' '[{"title":"Job"}]'
create_curl_sequence_mock() {
  local mock_path="$TEST_TMPDIR/bin"
  local seq_dir="$TEST_TMPDIR/curl_seq"
  mkdir -p "$mock_path" "$seq_dir"

  local i=0
  for resp in "$@"; do
    echo "$resp" > "$seq_dir/response_${i}"
    i=$((i + 1))
  done
  echo "$i" > "$seq_dir/total"
  echo "0" > "$seq_dir/call_index"

  cat > "$mock_path/curl" <<'MOCK'
#!/usr/bin/env bash
SEQ_DIR="${HOME}/.curl_seq_dir"
INDEX=$(cat "$SEQ_DIR/call_index")
RESPONSE=$(cat "$SEQ_DIR/response_${INDEX}" 2>/dev/null || echo '{}')
NEXT=$((INDEX + 1))
echo "$NEXT" > "$SEQ_DIR/call_index"
...
MOCK
  chmod +x "$mock_path/curl"
  # Link the sequence dir to a stable path the mock can find
  ln -sf "$seq_dir" "$HOME/.curl_seq_dir"
  export PATH="$mock_path:$PATH"
}
